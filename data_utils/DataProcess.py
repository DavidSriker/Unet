...
def combineMasks(left_mask, right_mask):
    ...
    # return combined_mask.reshape((1, flat_size)), combined_mask_dilate.reshape((1, flat_size))
...
def lungDataProcess(export_images=False):
    ...
    # print(all_images_df.sample(3))
    # print(all_images_df['path'].values[:4])
    ...
    # images_img_df = all_images_df[all_images_df['modality'] == 'CXR_png'].copy()
    # images_img_df['image_path'] = images_img_df['path'].map(str)
    #
    # images_seg_df = all_images_df[all_images_df['modality'] == 'masks'].copy()
    # images_seg_df['image_path'] = images_seg_df['path'].map(str)
    #
    # return images_img_df, images_seg_df
...
class LungSeg(Dataset):
    ...
    def __len__(self):
        return len(self.imgs) # of how many data(images?) you have
...
class LungSegVal(Dataset):
    ...
    def __getitem__(self, index):
        # image_name = self.imgs[index].split('/')[-1]
        image = Image.open(self.imgs[index])
        image = image.convert('L')
        mask = Image.open(self.seg[index])
        mask = mask.convert('L')
